<script lang="ts">
  ...
  import { Box, Button, Checkbox, Flex, Overlay, Text, TextInput, Title, createStyles, type theme } from "@svelteuidev/core";
  import { onMount } from "svelte";
  import { updateFileSecurityData } from "../../../server/files";
  import { isAxiosError } from "axios";
  import Card from "../homepage/Card.svelte";
...
export let visible = true;
  ...
  const sendData = async () => {
    ...
    try {
      await updateFileSecurityData(accessToken, file.id, is_file_encrypted, is_file_shared, current_file_password, new_file_password);
...
<Overlay opacity={1} color="#000" zIndex={5} center class={classes.flexOverlay}>
    <Box class={getStyles()}>
      <Flex direction="column" align="space-evenly" gap="md" justify="center">
        ...
        {#if !file.encrypted}
        <Flex justify="center" align="center" gap="md">
          <Checkbox visible={!file.encrypted}
          bind:checked={is_file_encrypted}></Checkbox>
          <Text>Encrypt?</Text>
        </Flex>
        {/if}
        {#if file.encrypted}
        ...
        {#if file.encrypted && changed_password}
          <Flex justify="center" align="center" gap="md">
            <Checkbox bind:checked={delete_password} on:click = { () => new_file_password = '' }></Checkbox>
        ...
        <Flex justify="space-around" align="center">
          <Button
            variant="filled"
            on:click={sendData}
            disabled={(file.encrypted && !current_file_password) || (changed_password && !new_file_password && !delete_password) ||
